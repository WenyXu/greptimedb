...
struct ProduceRequest {
    batch: Vec<Record>,
    sender: oneshot::Sender<ProduceResultHandle>,
}
...
fn handle_produce_requests(
    ...
    for ProduceRequest { batch, sender } in std::mem::take(requests) {
        ...
        let _ = sender.send(ProduceResultHandle { receivers });
    }
    pending_requests
}
...
async fn do_flush(
    client: &Arc<dyn ProducerClient>,
    PendingRequest { batch, sender }: PendingRequest,
    compression: Compression,
) {
    let result = client
...
impl OrderedBatchProducer {
    ...
    pub(crate) async fn produce(
        &self,
        batch: Vec<Record>,
    ) -> Result<oneshot::Receiver<ProduceResultHandle>> {
        let receiver = {
            let (tx, rx) = oneshot::channel();
            self.sender
