...
pub async fn register_logical_regions(
    metadata_region: &MetadataRegion,
    physical_region_id: RegionId,
    logical_regions: impl Iterator<Item = (RegionId, Vec<ColumnMetadata>)>,
) -> Result<()> {
    ...
    let iter = logical_regions
        ...
        .flat_map(|(logical_region_id, column_metadatas)| {
            ...
            .chain(column_metadatas.into_iter().map(move |column_metadata| {
                (
                    MetadataRegion::concat_column_key(
                        logical_region_id,
    ...
    let put_request = MetadataRegion::build_put_request_from_iter(iter);
    metadata_region
        .mito
        .handle_request(
            region_id,
